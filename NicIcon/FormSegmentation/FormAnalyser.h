...
class FormAnalyser
{
	private:
		...
		std::string currentImage, _outputDirectory, _outputFileList;
		std::map<std::string, std::vector<std::string> > _fileList;
		BaseForm m_baseForm;

	public:
		FormAnalyser(const std::string& outputDirectory, const std::string& outputFileList, const std::map<std::string, Icon*>& iconList, const std::map<std::string, std::string>& iconSizeList, const std::string& crossTemplate, const BaseForm& baseForm);
		~FormAnalyser();
